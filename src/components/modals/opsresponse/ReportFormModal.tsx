...
interface ReportFormModalProps {
  ...
  formData: ReportFormState;
  setFormData: React.Dispatch<React.SetStateAction<ReportFormState>>;
...
const ReportFormModal: React.FC<ReportFormModalProps> = ({
  ...
  const handleChange = (field: keyof ReportFormState, value: string | boolean) => {
    if (viewMode) return;
    setFormData((prev) => ({
      ...prev,
      [field]: value,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                disabled={viewMode}
                required
                placeholder="Enter report type (e.g., Monthly, Quarterly)"
              />
            </div>
