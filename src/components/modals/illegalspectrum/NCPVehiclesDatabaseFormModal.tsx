...
import { NCPVehiclesDatabaseFormState } from '../../illegalspectrum/NCPVehiclesDatabase';

interface NCPVehiclesDatabaseFormModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  formData: NCPVehiclesDatabaseFormState;
  setFormData: React.Dispatch<React.SetStateAction<NCPVehiclesDatabaseFormState>>;
...
const NCPVehiclesDatabaseFormModal: React.FC<NCPVehiclesDatabaseFormModalProps> = ({
  open,
  onOpenChange,
  formData,
  setFormData,
  onSubmit,
  title = 'Add NCP Vehicle to Database',
  submitLabel = 'Save',
  submitting = false,
  viewMode = false,
}) => {
  const handleChange = (key: keyof NCPVehiclesDatabaseFormState, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [key]: value,
    }));
      ...
      <form onSubmit={onSubmit} className="space-y-4 px-1 pb-1 max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Input
            ...
            onChange={(e) => handleChange('present_use', e.target.value)}
            disabled={viewMode}
          />
        </div>
        <div>
...
export default NCPVehiclesDatabaseFormModal;
