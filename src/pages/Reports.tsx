...
import ReportFormModal, { ReportFormState } from '../components/modals/opsresponse/ReportFormModal';
...
const buildInitialForm = (): ReportFormState => ({
  report_name: '',
  report_type: '',
  forwarded_to: '',
  ref_no: '',
...
const Reports: React.FC = () => {
  ...
  const [viewingRecord, setViewingRecord] = useState<Report | null>(null);
  ...
  const [formData, setFormData] = useState<ReportFormState>(buildInitialForm());
  ...
  const fetchRecords = async () => {
    try {
      ...
      const mappedData: Report[] = data.map((item: any) => ({
        ...
        ref_no: item.ref_no || '',
        is_feedback_recv: item.is_feedback_recv || false,
    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          ref_no: data.ref_no || '',
          is_feedback_recv: data.is_feedback_recv || false,
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-1">Manage reports</p>
              </div>
                ...
                {paginatedRecords.length === 0 ? (
                  ...
                  paginatedRecords.map((record) => (
                    <TableRow key={record._id || record.id}>
                      <TableCell className="font-medium">{record.report_name || 'N/A'}</TableCell>
                      <TableCell>{record.report_type || 'N/A'}</TableCell>
                      <TableCell>{record.forwarded_to || 'N/A'}</TableCell>
                      <TableCell>{record.ref_no || 'N/A'}</TableCell>
                      <TableCell>
      ...
      <ReportFormModal
        ...
        title={isViewMode ? 'View Report' : editingRecord ? 'Edit Report' : 'Add Report'}
        submitLabel={editingRecord ? 'Save Changes' : 'Add Report'}
